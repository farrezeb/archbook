...
timeout 90  'brightnessctl -s set 10%' resume 'brightnessctl -r'
timeout 150 'systemctl suspend'
EOF

    # AC / SERVIDOR: só apaga tela após 10min, nunca suspende, nunca bloqueia
...
switch_profile() {
    ...
    if is_on_ac; then
        ...
        sudo -u "$HYPR_USER" \
            ...
            HYPRLAND_INSTANCE_SIGNATURE="$sig" \
            swayidle -w -C /run/swayidle_ac.conf >/dev/null 2>&1 &
    else
        ...
        sudo -u "$HYPR_USER" \
            ...
            WAYLAND_DISPLAY="wayland-1" \
            HYPRLAND_INSTANCE_SIGNATURE="$sig" \
            swayidle -w -C /run/swayidle_bat.conf >/dev/null 2>&1 &
    fi
}
...
log "Monitorando eventos..."
{
    udevadm monitor --subsystem-match=power_supply --property &
    while true; do sleep 2; check_lid; done
