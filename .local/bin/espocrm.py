...
def cnae_descricao(codigo):
    """Retorna descrição de um código CNAE"""
    if not codigo or pd.isna(codigo):
        return ''
    c = str(codigo).strip()
    return CNAE_DESCRICAO.get(c, c)  # retorna o próprio código se não encontrar
...
def processar(entrada, saida):
    ...
    out = pd.DataFrame()
    out['name']                     = df['Razão Social'].str.strip()
    ...
    out['cCnaeSecundario']          = df['CNAE Secundário'].apply(cnae_secundario_descricao)
    ...
    out['emailAddress']             = df['E-mail Válido'].str.strip()
    out['billingAddressStreet']     = df.apply(make_street, axis=1)
    out['billingAddressCity']       = df['Município'].str.strip()
